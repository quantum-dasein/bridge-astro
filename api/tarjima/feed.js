...
// примерно один запрос в секунду независимо от того, десять человек на
// занятии или триста.
//
// Если кэш почему-то не сработает, всё продолжит работать — просто команд
// уйдёт больше.

export const config = { runtime: 'edge', regions: ['iad1'] }; // рядом с Upstash

// Куски короткие и идут часто; 40 хватает, чтобы телефон, потерявший связь
// на полминуты, не пропустил ничего.
const TAIL = 40;
...
function answer(body, cache) {
  return new Response(JSON.stringify(body), {
    ...
    headers: {
      ...
      // s-maxage=1 — CDN отвечает ему из своей копии не старше секунды.
      // Без stale-while-revalidate: он разрешал CDN отдавать ответ,
      // устаревший ещё на 4 секунды, — субтитры запаздывали на ровном месте.
      'cache-control': cache ? 'public, max-age=0, s-maxage=1' : 'no-store',
      'cdn-cache-control': cache ? 'max-age=1' : 'no-store',
    },
  });
}
