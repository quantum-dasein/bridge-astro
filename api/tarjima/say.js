...
Ответ — только узбекский текст перевода. Если в куске нет слов, которые можно перевести (шум, междометие, обрывок из пары букв), ответь одним символом ∅ — без извинений и пояснений: всё, что ты напишешь, слушатели увидят в субтитрах.
...
export default async function handler(req) {
  ...
  // Кусок без смысла (шум, «ээ») модель помечает ∅: в ленту и в Zoom он не
  // идёт, иначе слушатели прочли бы извинения модели вместо субтитра.
  if (!error && /^[∅\s.]*$/.test(uz)) skip = true;
  if (skip)
    return json({ ru: text, uz: '', skip: true, ms: Date.now() - started, modelMs, cost, tokens, model: variant, count });
